#!/bin/sh
g++ -o  make_cfg make_cfg.cpp -std=c++11
if [ ! -e ./make_cfg ]; then
	echo "compile error!"
	exit
fi

dz=0.1
g1=0.200
g2=1.200
w=0.400
Lsbend=10.0
echo -n > result.dat
echo "\t1.530\t1.540\t1.550\t1.560\t1.570" >> result.dat
L1=18.8
L2=4.8
Ldel1=6.2
Ldel2=30.0
for L in `seq 0.0 0.1 15.0`
do  
    echo -n "${L}" >> result.dat

    Ldel=`echo "scale=3;15.0 - ${L}" | bc`
    ./make_cfg ${dz} ${g1} ${w} 11 dc ${L1} 1 dc ${Ldel1} 4 dc ${L2} 1 sbend ${Lsbend} ${g2} 1 dc ${Ldel} 9 dc ${L} 8 dc 15.0 8 sbend ${Lsbend} ${g1} 1 dc ${L2} 1 dc ${Ldel1} 2 dc ${L1} 1

    for wl in `seq 1.530 0.010 1.570`
    do
        ~/msh/CMT_sbend/cmt -pcm9 -wl ${wl} -zw
        echo -n "\t`tail -n1 output | head -n1 | cut -f3 `" >> result.dat
    done
    echo >> result.dat
done
