...
int main(int argc, char *argv[]){
	...
	auto msh_xy = [&](int tp_num, double Z) -> void {
		...
		Copy(Lin, 0.0, Wpml, 0.0, 			"1", &func);
		Copy(Lin, 0.0, Hsio2, 0.0, 			"2", &func);
		Copy(Lin, 0.0, Hsi+Hpcm+Wm, 0.0, 	"5", &func);
		Copy(Lin, 0.0, Wpml, 0.0, 			"8", &func);

		Line(W-Wpml, 0.0, Z, 	W, 0.0, Z, &func);
		Copy(Lin, 0.0, Wpml, 0.0, 			"14", &func);
		Copy(Lin, 0.0, Hsio2, 0.0, 			"15", &func);
		Copy(Lin, 0.0, Hsi+Hpcm+Wm, 0.0, 	"18", &func);
		Copy(Lin, 0.0, Wpml, 0.0, 			"21", &func);

		Copy(Lin, W-2*Wpml, 0.0, 0.0, 		"4 13", &func);

		Copy(Poi, Wm, 0.0, 0.0, 			"6", &func);
		Copy(Poi, Wr, 0.0, 0.0, 			"21", &func);
		Copy(Poi, g, 0.0, 0.0, 				"22", &func);
		Copy(Poi, Wh, 0.0, 0.0, 			"23", &func);
		Copy(Poi, Wm, 0.0, 0.0, 			"24", &func);

		Copy(Lin, 0.0, Hsi, 0.0, 			"32", &func);

		Copy(Poi, 0.0, Hsi, 0.0, 			"23 24", &func);
		Copy(Poi, (Wh-Wpcm)/2, 0.0, 0.0, 	"27", &func);
		Copy(Poi, Wpcm, 0.0, 0.0, 			"29", &func);
		Copy(Poi, (Wh-Wpcm)/2, 0.0, 0.0, 	"30", &func);

		Surface("34 39 41 42 43 40", &func);

		Copy(Lin, 0.0, Hpcm, 0.0, 			"42", &func);
		Surface("28 7 31 32 33 34 35 19", &func);
		Surface("10 29 22 35 40 43 46 44 45 41 39 33 38 36 37 31", &func);

		Tppush(0, 15, 46, 32, &func);
	};
